...
- 12e9b54f0e: Added back support for when users do not provide a branch to the `UrlReader` for Bitbucket Server
...
- 5e4dc173f7: Added a second validation to the `dir()` method of ZIP archive responses returned from `readTree()` that ensures that extracted files don't fall outside the target directory.
...
- cfa078e255: The `ZipArchiveResponse` now correctly handles corrupt ZIP archives.
  ...
  Before this change, certain corrupt ZIP archives either cause the inflater to throw (as expected), or hang the parser indefinitely.

  By switching out the `zip` parsing library, we now write to a temporary directory, and load from disk which should ensure that the parsing of the `.zip` files is done correctly because `streaming` of `zip` paths is technically impossible without being able to parse the headers at the end of the file.

- 770d3f92c4: The config prop `ensureExists` now applies to schema creation when `pluginDivisionMode` is `schema`. This means schemas no longer automatically create when `ensureExists` is `false`.
...
- d0eefc499a: Made the `to` prop of `Button` and `Link` more strict, only supporting plain strings. It used to be the case that this prop was too liberal, making it look like we supported the complex `react-router-dom` object form of the parameter as well, which led to unexpected results at runtime.
...
- 29f782eb37: Updated dependency `@types/luxon` to `^3.0.0`.
- 1f27d83933: Fixed bug in getGitLabFileFetchUrl where a target whose path didn't contain the
  `/-/` scope would result in a fetch URL that didn't support
...
- bfc7c50a09: Display associated entity as a chip in `AdrSearchResultListItem`
  ...
  BREAKING: `AdrDocument` now includes a `entityRef` property. If you have a custom `AdrParser`, you need to supply this property in your returned documents
...
- bfc7c50a09: Display associated entity as a chip in `AdrSearchResultListItem`
  ...
  BREAKING: `AdrDocument` now includes a `entityRef` property, if you have a custom `AdrParser` you must supply this property in your returned documents
...
- 80da5162c7: Plugin catalog now uses an experimental feature where you can customize the title of the create button.
...
- f7607f9d85: Add new plugin catalog-backend-module-bitbucket-server which adds the `BitbucketServerEntityProvider`.
  ...
  The entity provider replaces the `BitbucketDiscoveryProcessor` for Bitbucket Server (Bitbucket Cloud already has a replacement).
...
- 347ea327c2: Moved communication with GitHub graphql API to the dedicated plugin API.
  Fixes issue when no GitHub Issues appear when GitHub API partially fails.
...
- c971afbf21: The `publish:file` action has deprecated in favor of testing templates using the template editor instead. Note that this action isn't and was never installed by default.
- b10b6c4aa4: Fix issue on Windows where templated files where not skipped as intended.
...
- 619b515172: **BREAKING** This plugin now call the `sonarqube-backend` plugin instead of relying on the proxy plugin
  ...
  The whole proxy's `'/sonarqube':` key can be deleted from your configuration files.
...
- 0599732ec0: Refactored experimental backend system with new type names.
- 34c2f5aca1: The factory returned by `createBackendPlugin` and `createBackendModule` no longer requires passing a parameter if the options are optional.
- Updated dependencies
  - @backstage/backend-common@0.15.0
  - @backstage/backend-tasks@0.3.4
...
- 80da5162c7: Introduced a new experimental feature that allows you to declare plugin-wide options for your plugin by defining
  ...
  This is an experimental feature and it has breaking changes in the future.

- 87649a06bf: Add a note that the `fetchApi` utility shouldn't run on sign-in page implementations and similar.
...
- b50e8e533b: Add an `$openapi` placeholder resolver that supports more use cases for resolving `$ref` instances. This means that the recently added `OpenApiRefProcessor` is deprecated in favor of the `openApiPlaceholderResolver`.

  An example demonstrates how to use it below.
...
- a641f79dcb: Move CSS overflow property to quadrant block element (that is to a div element) in RadarLegend component.
- Updated dependencies
  - @backstage/core-components@0.11.0
  - @backstage/core-plugin-api@1.0.5
...
- e924d2d013: Added back reduction in size, this fixes the large TeachDocs headings
...
In this release, the developers introduced a new timeout configuration for event bus polling requests, enhancing the event handling reliability and offering increased flexibility for users.

**Parameter Explanation**:
- **`notifyTimeoutMs`**: Sets the timeout for event bus polling requests in milliseconds. The default value is 55 seconds.

**Example Configuration**:
```yaml
events:
  notifyTimeoutMs: 60000 # Timeout for event bus polling requests in milliseconds
```

**Use Case**: 
Adjusting the `notifyTimeoutMs` parameter can be beneficial in scenarios to prevent stalls or when adapting to specific network conditions.
...
A new constant, EVENTS_NOTIFY_TIMEOUT_HEADER, now enhances the event bus router. This constant plays a crucial role in the configuration of event polling and request handling.

**Behavioral Changes**:
- The event polling mechanism now respects the configured or default timeout.
- The `EVENTS_NOTIFY_TIMEOUT_HEADER` facilitates better request handling by ensuring that the system acknowledges the specified timeout during operations.
...
This update introduces a configurable timeout for event bus polling requests through the `notifyTimeoutMs` parameter in the `app-config.yaml`. It enhances event handling reliability and allows users to customize their timeout settings to suit specific network conditions or prevent stalls.

**Instructions for Developers**:
- Update your `app-config.yaml` to include the new `notifyTimeoutMs` parameter as needed.
- Review any dependencies or custom implementations that interact with the event bus to ensure compatibility with the new timeout settings.

By following these updates, users, and developers can utilize the newly introduced timeout feature, leading to more robust and flexible application configurations.# Release v1.5.0
...
By following these updates, users, and developers can utilize the newly introduced timeout feature, leading to more robust and flexible application configurations.